  ...
  function($http, $location, $q, $filter, $stateParams){
    ...
    var queryDataCacheWithAnArrayOfIds = function(ids){
      ...
      queryBackend(featureService.dataCache, queryParams)
        .then(function(dataCacheResults){
          ...
          for (var i = 0; i < dataCacheResults.features.length; i++) {
            var attributes = dataCacheResults.features[i].attributes;
            if(attributes.type === 'ZONING'){
              if(dataCacheProperties.zoning !== undefined){
                dataCacheProperties.zoning[attributes.civicaddress_id] = createArrayFromNullorString(attributes.data, ',');
              }else{
                dataCacheProperties.zoning = {};
                dataCacheProperties.zoning[attributes.civicaddress_id] = createArrayFromNullorString(attributes.data, ',');
              }
              //dataCacheProperties.zoning = createArrayFromNullorString(attributes.data, ',');
            }else if(attributes.type === 'CRIME'){
    ...
    var formatPropertyData = function(property){
      ...
      for (var p = 0; p < property.features.length; p++) {
        ...
        if(dataCacheProperties.zoningOverlays){
          var zoningOverlaysSplit = dataCacheProperties.zoningOverlays.split('-');
          var zoningOverlayParams = {
           ...
           queryBackend(featureService.zoningOverlay, zoningOverlayParams)
              .then(function(zoningOverlayLayer){
                ...
                geojson.overlays = overlayGeojson;
                q.resolve(geojson);
              });
        }else{
          q.resolve(geojson);
    ...
    Topic.zoning = function(){
      ...
      var geojson = {
        ...
        'features' : [{
            ...
            'geometry' : {
              'type' : 'Point',
              'coordinates' : [dataCacheProperties.searchGeojson.features[0].geometry.coordinates[0], dataCacheProperties.searchGeojson.features[0].geometry.coordinates[1]]
            }
        }]
      };
      if(dataCacheProperties.zoningOverlays){
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('topics/topics.list.html',
    '<div><div class="col-xs-12 btn-group btn-group-justified" style="margin-top : 15px"><a href="#/search/composite" class="btn btn-primary"><i class="fa fa-search"></i> <strong>Search</strong></a></div><div class="col-xs-12"><hr></div><h2 class="text-muted text-center">{{heading}}</h2><h1 class="text-muted text-center">{{searchText}}</h1><h3 ng-if="anAddress && loading" class="text-muted text-center"><i class="fa fa-lg fa-spinner fa-spin text-muted"></i></h3><h3 ng-if="anAddress && inTheCity && !loading" class="text-muted text-center"><i class="fa fa-lg fa-check-circle text-success"></i> <strong>It\'s in the city!</strong></h3><h3 ng-if="anAddress && !inTheCity && !loading" class="text-muted text-center"><i class="fa fa-lg fa-times-circle text-danger"></i> <strong>It\'s not in the city!</strong></h3><div class="list-group" style="margin-top : 20px"><a class="row list-group-item list-item-panel" href="{{topic.linkTo}}" style="margin-bottom : 5px" ng-repeat="topic in topics"><span class="visible-xs col-xs-8"><p class="text-primary text-center">{{topic.question}}</p></span> <i ng-class="topic.iconClass" class="visible-xs pull-left col-xs-3 text-primary"></i><div ng-class="topic.iconClass" class="hidden-xs col-sm-2 text-primary"></div><h4 class="hidden-xs col-sm-9 text-primary" style="margin-top : 20px">{{topic.question}}</h4><h4 class="col-sm-1 hidden-xs"><i class="fa fa-2x fa-chevron-right text-primary pull-right"></i></h4></a></div><div class="col-xs-12 text-center">List icon font generated by <a href="http://www.flaticon.com">flaticon.com</a> under <a href="http://creativecommons.org/licenses/by/3.0/">CC</a> by <a href="http://www.zurb.com">Zurb</a>, <a href="http://www.freepik.com">Freepik</a>, <a href="http://www.unocha.org">OCHA</a>.</div></div>');
}]);
})();
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('topic/topic-headers/topic.header.in.html',
    '<div class="list-item-panel col-xs-12"><h1>{{topicProperties.title}}</h1><form class="form-horizontal"><div class="form-group"><label class="col-sm-2 control-label"><h3 style="margin : 0px"><strong class="text-muted">in</strong></h3></label><div class="col-sm-10"><h3 class="form-control-static" style="margin : 0px">{{stateParams.searchtext}} Neighborhood</h3></div></div></form></div>');
}]);
})();
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('topic/topic-headers/topic.header.ownedby.html',
